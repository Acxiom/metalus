...
object PubSubSteps {
  ...
  def postMessage(message: String, topicName: String, credentials: Option[Map[String, String]] = None, attributes: Option[Map[String, String]]): Unit = {
  ...
  def postMessage(message: String, topicName: String, pipelineContext: PipelineContext): Unit = {
    GCPUtilities.postMessageInternal(topicName, GCPUtilities.getCredentialsFromPipelineContext(pipelineContext), message)
  }
