...
object JSONSteps {
  ...
  def jsonStringToMap(jsonString: String, formats: Option[Json4sSerializers] = None): Option[Map[String, Any]] = {
    val map = JsonParser.parseMap(jsonString, formats)
    if (Option(map).isDefined) {
      Some(map)
    } else {
      None
    }
  ...
  @StepParameters(Map(
    ...
    "formats" -> StepParameter(None, Some(false), None, None, None, None, Some("Json4sSerializers object that will override the pipeline context formats"))))
  @StepResults(primaryType = "String",
    secondaryTypes = None)
  def jsonMapToString(jsonMap: Map[String, Any], formats: Option[Json4sSerializers] = None): String =
    JsonParser.serialize(jsonMap, formats)
  ...
  @StepResults(primaryType = "String",
    secondaryTypes = None)
  def objectToJsonString(obj: AnyRef, formats: Option[Json4sSerializers] = None): String =
    JsonParser.serialize(obj, formats)
  ...
  def jsonStringToObject(jsonString: String,
                         objectName: String,
                         formats: Option[Json4sSerializers] = None): Any = JsonParser.parseJson(jsonString, objectName, formats)
  ...
  @StepParameters(Map(
    ...
    "formats" -> StepParameter(None, Some(false), None, None, None, None, Some("Json4sSerializers object that will override the pipeline context formats"))))
  def jsonStringToSchema(schema: String, formats: Option[Json4sSerializers] = None): Schema =
    JsonParser.parseJson(schema, "com.acxiom.metalus.sql.Schema", formats).asInstanceOf[Schema]
