...
object MockStepObject {
  ...
  def alternateMockStepFunction(string: String, boolean: Boolean): PipelineStepResponse = {
    PipelineStepResponse(Some(s"ALT_$string"), Some(Map[String, Any]("boolean" -> boolean, "string" -> string)))
  }
  ...
  override def pipelineContext: PipelineContext = {
    PipelineContext(None, List[PipelineParameter](),
      Some(if (parameters.contains("stepPackages")) {
        ...
        List("com.acxiom.metalus.steps", "com.acxiom.metalus")
      }),
      PipelineStepMapper(),
