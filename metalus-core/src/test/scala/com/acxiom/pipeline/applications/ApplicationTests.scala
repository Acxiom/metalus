...
import com.acxiom.pipeline.Constants.{NINE, TWELVE}
...
//    }
    ...
    it("Should load and create an Application from an http location") {
      ...
      val application = JsonParser.parseJson(applicationJson, "com.acxiom.pipeline.applications.Application").asInstanceOf[Application]
...
//    }
    ...
    it("Should parse formats") {
      val jsonString =
        """{
          ...
          |  ]
          |}""".stripMargin
      val serializers = JsonParser.parseJson(jsonString, "com.acxiom.pipeline.applications.Json4sSerializers")(DefaultFormats).asInstanceOf[Json4sSerializers]
  ...
  private def verifyMappedParameter(pipeline: Pipeline) = {
    ...
    assert(pipeline.steps.get.head.params.get(1).className.getOrElse("") == "com.acxiom.pipeline.applications.TestGlobalObject")
