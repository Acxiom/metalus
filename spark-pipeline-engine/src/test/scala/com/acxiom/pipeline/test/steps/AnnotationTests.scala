...
class AnnotationTests extends FunSpec {
  describe("Step Annotation Tests") {
    it("Should find step object in provided package") {
      ...
      assert(step1.get.parameters.isEmpty)
      ...
      assert(step2.get.parameters.nonEmpty)
      assert(step2.get.parameters.lengthCompare(2) == 0)
      val param1 = step2.get.parameters.find(p => p.name == "id")
      assert(param1.isDefined)
      assert(param1.get.`type` == "text")
      val param2 = step2.get.parameters.find(p => p.name == "num")
