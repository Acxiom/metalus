...
class GlobalStepUpdateTests extends FunSpec with BeforeAndAfterAll with Suite {
  ...
  describe("GlobalValue Manipulation from fork steps") {
    val pipelineJson =
      """
        |[
        | {
        ...
        |   "steps": [
        |   {
        |       "id": "Pipeline1Step1Fork",
        |       "displayName": "Pipeline1Step1Fork",
        |       "type": "fork",
        |       "nextStepId": "Pipeline1Step1",
        |       "params": [
        |         {
        |           "type": "text",
        |           "name": "forkByValues",
        |           "required": true,
        |           "value": ["redonthehead", "redonthehead1"]
        |         },
        |         {
        |           "type": "text",
        |           "name": "forkMethod",
    ...
    it("Should allow steps to add a global") {
      ...
      assert(ctx.globals.get.size == 4)
      assert(ctx.globals.get.contains("redonthehead"))
      assert(ctx.globals.get("redonthehead").isInstanceOf[List[Any]])
      assert(ctx.globals.get("redonthehead").asInstanceOf[List[Any]].length == 2)
      assert(ctx.globals.get("redonthehead").asInstanceOf[List[Any]].head == Some("fred"))
      assert(ctx.globals.get("redonthehead1").isInstanceOf[List[Any]])
      assert(ctx.globals.get("redonthehead1").asInstanceOf[List[Any]].length == 2)
      assert(ctx.globals.get("redonthehead1").asInstanceOf[List[Any]](1) == Some("fred"))
    }
    ...
    it("Should allow steps to overwrite a global") {
      ...
      assert(ctx.globals.get("redonthehead").asInstanceOf[List[Any]].length == 2)
      assert(ctx.globals.get("redonthehead").asInstanceOf[List[Any]].head == Some("fred"))
      assert(ctx.globals.get("redonthehead").asInstanceOf[List[Any]](1) == Some("fred"))
    }
  }
}
